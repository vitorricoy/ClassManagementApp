...
import { getMeanStudent, getRepetitionMaterial, getRepetitionMaterialStudent, ProgressMeanStudent, ProgressRepetitionMaterial, ProgressRepetitionMaterialStudent } from "../../../api/progress";

const ProgressMeanStudentPlot = () => {
  const [progressMeanStudent, setProgressMeanStudent] = React.useState<ProgressMeanStudent[]>([]);
  ...
  const getPlot = () => {
    return <Plot
      ...
      layout={{
        title: 'Número médio de vezes que um material é repetido por aluno (dentre os materiais que ele viu ao menos uma vez)',
        margin: {
          t: 50,
          b: 250,
          l: 50,
  ...
  return progressMeanStudent.length ? getPlot() : <div className="spinner"><Spin /></div>;
};

const ProgressRepetitionMaterialStudentPlot = () => {
  const [progressRepetition, setProgressRepetition] = React.useState<ProgressRepetitionMaterialStudent[]>([]);
  ...
  return progressRepetition.length ? getPlot() : <div className="spinner"><Spin /></div>;
};

const ProgressHistogramStudentPlot = () => {
  const [progressHistogramStudent, setProgressHistogramStudent] = React.useState<number[]>([]);
  ...
  const getPlot = () => {
    const maxRepetition = progressHistogramStudent ? Math.max(...progressHistogramStudent) : 0;
    return <Plot
      style={{ paddingTop: '20px' }}
      data={[
        {
      ...
      layout={{
        title: 'Histograma da repetição média da visualização de materiais por aluno',
        bargap: 0.1,
        margin: {
          t: 50,
          b: 50,
  ...
  return progressHistogramStudent.length ? getPlot() : <div className="spinner"><Spin /></div>;
};

const ProgressRepetitionMaterialPlot = () => {
  const [progressRepetition, setProgressRepetition] = React.useState<ProgressRepetitionMaterial[]>([]);
...
const ProgressHistogramMaterialPlot = () => {
  const [progressHistogramMaterial, setProgressRepetition] = React.useState<number[]>([]);
  ...
  const getPlot = () => {
    const maxRepetition = progressHistogramMaterial ? Math.max(...progressHistogramMaterial) : 0;
    return <Plot
      style={{ paddingTop: '20px' }}
      data={[
        {
  ...
  return progressHistogramMaterial.length ? getPlot() : <div className="spinner"><Spin /></div>;
};
...
const DashboardPlotMenu = ({ currentItem, setCurrentItem }: { currentItem: string; setCurrentItem: (v: string) => void; }) => {
  return (
    ...
    >
      <Menu.Item
        key="mean_repetition_student"
      >
        Repetição média de material por aluno
      </Menu.Item>
      <Menu.Item
        key="repetition_material_student"
      >
      ...
      <Menu.Item
        key="repetition_material"
      >
        Repetição média por material
      </Menu.Item>
      <Menu.Item
        key="histogram_material"
      >
...
export const Dashboard = () => {
  const [currentItem, setCurrentItem] = React.useState<string>("mean_repetition_student");

  const getCurrentItemComponent = () => {
    const map = new Map<string, JSX.Element>([
      ['mean_repetition_student', <ProgressMeanStudentPlot />],
      ['repetition_material_student', <ProgressRepetitionMaterialStudentPlot />],
