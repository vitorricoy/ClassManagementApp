...
export const getDeliveryHeatmap = async () => {
  const { data, status } = await axios.get<DeliveryHeatmap>(
    BASE_URL + "delivery/heatmap",
    {
      params: {
...
export const getDeliveryStudentCount = async () => {
  const { data, status } = await axios.get<DeliveryStudentCount[]>(
    BASE_URL + "delivery/student_count",
    {
      params: {
        token: localStorage.getItem("userToken"),
        class_code: localStorage.getItem("classCode"),
...
export const getDeliveryActivityCount = async () => {
  const { data, status } = await axios.get<DeliveryActivityCount[]>(
    BASE_URL + "delivery/activity_count",
    {
      params: {
        token: localStorage.getItem("userToken"),
        class_code: localStorage.getItem("classCode"),
